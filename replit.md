...
- **Voicemail System**: Supports both default and personalized voicemails. Personalized voicemails utilize ElevenLabs TTS with SSML for advanced speech processing. AMD uses Telnyx `premium` mode. Beep detected → voicemail drops immediately. No beep detected → system waits 4s for transcription to classify as gatekeeper vs voicemail. AI gatekeeper detection via transcription keywords (e.g. "reason for calling", "I'll see if", "please stay on the line") triggers call transfer instead of voicemail drop. Voicemail keywords (e.g. "leave a message", "after the beep") trigger voicemail drop with 5s delay. 30s fallback timer for machine result if no beep/transcription classification. 60s safety timeout hangs up calls with no action taken.
- **AI Gatekeeper Detection**: When AMD detects "machine" but transcription reveals an AI receptionist (not a voicemail system), the system cancels voicemail drop and transfers the call to the configured transfer number instead. Detection happens both in the no-beep fallback timer (4s after greeting.ended) and in real-time transcription analysis. Gatekeeper keywords include: "reason for calling", "who is calling", "I'll see if", "please stay on the line", "how can I help", etc.
