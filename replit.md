...
- **Voicemail System**: Supports both default and personalized voicemails. Personalized voicemails utilize ElevenLabs TTS with SSML for advanced speech processing. AMD uses Telnyx `detect_beep` mode to wait for the actual beep before dropping voicemail. Greeting duration tolerance set to 30s, after-greeting silence 1.5s. Fallback timer is 30s (drops voicemail if beep event never arrives). Transcript keyword detection adds 3s delay after keyword match to allow beep to sound.
